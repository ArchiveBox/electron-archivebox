...
const captureRealScreens = async ({ dataDir, userDataDir, port, containerName }) => {
    ...
    try {
        ...
        const imagePull = page.locator('#service-panel[data-state="pulling"]').waitFor({ state: 'attached' })
        ...
        await frame.getByRole('button', { name: /^HTML, .* outputs, expand stack$/ }).click()
        const wgetPreview = frame.getByRole('link', { name: /^Wget wget / })
        await wgetPreview.waitFor()
